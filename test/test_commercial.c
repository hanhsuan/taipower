...
int test_commercial() {
  ...
  assert((non_time_of_use_charge_calc(kwh, summer_info) - 253) <
         TAIPOWER_FLOAT_TOLERANCE);

  assert((non_time_of_use_charge_calc(kwh, non_summer_info) - 212) <
         TAIPOWER_FLOAT_TOLERANCE);

  kwh = 350;

  assert((non_time_of_use_charge_calc(kwh, summer_info) - 905.9) <
         TAIPOWER_FLOAT_TOLERANCE);

  assert((non_time_of_use_charge_calc(kwh, non_summer_info) - 757.8) <
         TAIPOWER_FLOAT_TOLERANCE);

  kwh = 750;

  assert((non_time_of_use_charge_calc(kwh, summer_info) - 2360.9) <
         TAIPOWER_FLOAT_TOLERANCE);

  assert((non_time_of_use_charge_calc(kwh, non_summer_info) - 1948.3) <
         TAIPOWER_FLOAT_TOLERANCE);

  kwh = 1750;

  assert((non_time_of_use_charge_calc(kwh, summer_info) - 7155.9) <
         TAIPOWER_FLOAT_TOLERANCE);

  assert((non_time_of_use_charge_calc(kwh, non_summer_info) - 5790.8) <
         TAIPOWER_FLOAT_TOLERANCE);
